...
player = FirstPersonController(model = 'player_1.obj',texture = player_texture,shader = basic_lighting_shader,double_sided=True)
player.gravity =0.4
player.y = 10
player.origin_y = 0.1
player.speed = 7
...
chunk_size = 1
render_distance = 4# render distance in chunks
chunks = {}
    ...
    for i in range(chunk_size):
        for j in range(chunk_size):
            ...
            for k in range(-1, -3, -1):
                chunk.append(create_block((world_x, -k-15+c, world_z), tx1))
...
def Tree_update():
    for t  in range (30):
        ...
        if i == 1:
            x =random.randint(-100,100) -player.position.x
            z =random.randint(-100,200) +player.position.x
            Tree(position=  (x,1,z))
            Tree(position=  (x,2,z))
            Tree(position=  (x,3,z))
